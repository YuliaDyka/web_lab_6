...
const ItemInfo = () => {
  const { id } = useParams();
  const [ordersData, setOrdersData] = useContext(OrdersContext);
  // console.log("========== id: " + id);
  const order = ordersData.find((ord) => ord.id === id);

  // for (const ordrr of ordersData) {
  //   console.log("-------- " + ordrr.id);
  //   for (const prop in ordrr) {
  //     console.log("   *** prop: " + prop + ",  val: " + ordrr[prop]);
  //   }
  // }
  ...
  return (
    <>
      <Card sx={{ maxWidth: 400, padding: 10 }}>
        <CardMedia component="img" height="140" image={order.img} />
        <CardContent>
